...
import com.myapp.iib.model.DSNModel;
...
import static com.myapp.iib.model.DSNModel.*;

@PageTitle("DSNs Used")
@Route(value = "dsns", layout = MainLayout.class)
public class DSNsUsed extends ViewFrame {
    ...
    public void load() {
        ...
        wrapperBox.addAll(FlowUtilsWrapper.dsnsUsedListing(flowUtils.allMFs()));
        listing.refresh();
    ...
    private Component createListing() {
        ...
        dataProvider.addFilter(propertyBox -> MyUtils.addFilter(searchField, propertyBox,
                NAME, DSNModel.DSN, NODE_NAME, NODE_TYPE, PARENT_NAME, EG_NAME)
        );


        listing = Components.listing.properties(SNO, NAME, DSNModel.DSN, NODE_NAME, NODE_TYPE)
                ...
                .itemDetailsComponent(propertyBox -> {
                    PropertyInputForm inputForm = Components.input.form(PARENT_NAME, EG_NAME)
                            .initializer(layout -> {
                                layout.setWidthFull();
                            })
