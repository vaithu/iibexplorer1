...
import static com.myapp.iib.model.URLsUsedModel.*;

@PageTitle("URLs Used")
@Route(value = "urls", layout = MainLayout.class)
public class URLsUsed extends ViewFrame {
    ...
    public void load() {
        ...
        wrapperBox.addAll(FlowUtilsWrapper.urlsUsedListing(node,flowUtils.allMFs()));
        listing.refresh();
    ...
    private Component createListing() {
        ...
        dataProvider.addFilter(propertyBox -> MyUtils.addFilter(searchField, propertyBox,
                NAME,URL,PROXY,NODE_NAME,NODE_TYPE,PARENT_NAME,EG_NAME)
        );
        ...
        listing = Components.listing.properties(SNO,NAME,URL)
                ...
                .itemDetailsComponent(propertyBox -> {
                    PropertyInputForm inputForm = Components.input.formHorizontal(NODE_NAME, NODE_TYPE, PROXY, PARENT_NAME, EG_NAME)
                            .initializer(layout -> {
                ...
                .withComponentColumn(propertyBox -> {
                    boolean isInput = propertyBox.getValue(NODE_TYPE).contains("Input");
