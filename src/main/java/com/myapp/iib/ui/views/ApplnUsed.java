...
import static com.myapp.iib.model.ApplnModel.*;

@PageTitle("Applications Used")
@Route(value = "applns", layout = MainLayout.class)
public class ApplnUsed extends ViewFrame {
    ...
    public void load() {
        ...
        wrapperBox.addAll(FlowUtilsWrapper.applnListing(flowUtils.allApplications()));
        listing.refresh();
    ...
    private Component createListing() {
        ...
        dataProvider.addFilter(propertyBox -> MyUtils.addFilter(searchField, propertyBox,
                NAME, EG_NAME,SHALED_LIB_DEPENDENCIES,STATIC_LIB,SHALED_LIB_DEPENDENTENTS)
        );


        listing = Components.listing.properties(SNO, NAME,EG_NAME, SHALED_LIB_DEPENDENCIES,STATIC_LIB,SHALED_LIB_DEPENDENTENTS)
                ...
                .itemDetailsComponent(propertyBox -> {
                    PropertyInputForm inputForm = Components.input.form(EG_NAME)
                            .initializer(layout -> {
                                layout.setWidthFull();
                            })
