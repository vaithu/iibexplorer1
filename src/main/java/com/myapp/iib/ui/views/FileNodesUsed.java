...
import static com.myapp.iib.model.FilesNodeModel.*;

@PageTitle("File Nodes Used")
@Route(value = "filesNodes", layout = MainLayout.class)
public class FileNodesUsed extends ViewFrame {
    ...
    public void load() {
        ...
        wrapperBox.addAll(FlowUtilsWrapper.filesNodesUsedListing(flowUtils.allMFs()));
        listing.refresh();
    ...
    private Component createListing() {
        ...
        dataProvider.addFilter(propertyBox -> MyUtils.addFilter(searchField, propertyBox,
                NAME, NODE_NAME, NODE_TYPE, LOCAL_DIR, PATTERN
                , FTP, FTP_TYPE, FTP_USER, FTP_DIR, FTP_MODE, FTP_SCAN_DELAY, PARENT_NAME, PARENT_TYPE, EG_NAME)
        );


        listing = Components.listing.properties(SNO, NAME, NODE_NAME, NODE_TYPE, LOCAL_DIR, PATTERN)
                ...
                .itemDetailsComponent(propertyBox -> {
                    PropertyInputForm inputForm = Components.input.form(FTP, FTP_TYPE, FTP_USER, FTP_DIR, FTP_MODE, FTP_SCAN_DELAY, PARENT_NAME, PARENT_TYPE, EG_NAME)
                            .initializer(layout -> {
                                layout.setWidthFull();
                            })
